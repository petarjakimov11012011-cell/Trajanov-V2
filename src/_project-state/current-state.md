NEXT: **⚠ RUN `supabase db push` — the Y.06 migration is STILL not on hosted, and until it is, `main` and the production database disagree (owed **#68**, `D-Y.06-11`). Y.07 did not touch it and did not run it.** **Then [P2] `/impeccable polish` + the closing `/impeccable audit` — on a NEW branch (`D-2.25-26`).** **Phase Y.07 — Composition & care copy — COMPLETE (2026-08-27, branch `phase-Y.07-care-copy`, PR [#42](https://github.com/petarjakimov11012011-cell/Trajanov-V2/pull/42), awaiting operator review — NOT merged).** Vladimir confirmed **100% cotton, wash at 30 °C** (via Petar, 2026-08-27). `facts.md` §7 moves **UNVERIFIED — OWED → VERIFIED**, with the provenance written into the row as the **owner's statement, explicitly NOT a label read** (`D-Y.07-1`) — **if a label is ever photographed and disagrees, the label wins and `D-Y.07-1` is what gets superseded.** The one statement covers **all three colourways**, which **amends `D-Y.02-1`** (`D-Y.07-2`). `careMk`/`careEn` are filled per product (no shared constant, no default — `D-Y.07-3`), so the slot Y.06 built now renders: MK „100% памук. Перење на 30 °C." / EN "100% cotton. Wash at 30 °C.", U+00A0 before `°C`. **Verified locally on all six product pages, both locales, at 390px and 1180px:** placeholder gone from all six, correct locale on each, styled identically to the adjacent Shipping body (`text-muted-foreground text-small`, 13px/19.5px, `rgb(171,167,158)`), `30 °C` on one line (also checked at 320px). `productJsonLd` still emits **zero** `Product` nodes; `src/lib/product-care.ts` byte-unchanged; `Placeholder.composition` and the null → placeholder branch both **kept** — the next drop starts null and must still say so. **Placeholder rows #3 and #9 CLEARED** (the first to close since 2.05); **#2/#4/#6/#7/#8/#10 still OPEN** and the register must still reach zero before the first REAL drop. Y.06's guard is **replaced, not deleted**, by an exact-string allowlist (`D-Y.07-4`, superseding `D-Y.06-9`) — watched failing on a deliberately wrong string, then reverted. **177/177 tests across 25 files** (176/25 at Y.06); build, lint and `tsc` clean. New owed **#69** (the two strings on production after deploy — care copy is config, so **merging is not shipping**) and **#70** (native MK check, precedent row **#66** still open). `supabase/migrations/` untouched. Nothing on the critical path moved: **Y.01** (drop content load) is still the gate.
...
**Phase Y.07 — Composition & care copy — COMPLETE (2026-08-27, branch `phase-Y.07-care-copy`,
PR [#42](https://github.com/petarjakimov11012011-cell/Trajanov-V2/pull/42) — open, awaiting operator review, **not merged**; report:
`src/_project-state/completions/Part-Y-Phase-07-Completion.md`).** The slot Y.06 shipped inert now carries a fact. Vladimir confirmed **100% cotton** and a
...
| 69 | **The two care strings on PRODUCTION, both locales, all three product pages, after the Y.07 deploy.** Care copy is **config, not a database column** (`D-Y.06-1`), so it does not exist for a customer until `main` is deployed — **merging is not shipping**, and `npm run sync:drop` will not carry it. Verified locally on six pages at 390px and 1180px; never once on `https://www.trajanovv.com`. **Pass:** on `/katalog/test-mustard-ochre`, `/katalog/test-off-white`, `/katalog/test-baby-blue` and their `/en/catalog/…` twins — `[PLACEHOLDER: состав и нега — од етикетата]` is **gone from all six**, the correct locale's string is present (MK „100% памук. Перење на 30 °C." / EN "100% cotton. Wash at 30 °C."), it reads like the Shipping body beside it rather than a badge or a heading, and **`30 °C` does not wrap** at 390px. | Y.07 | **Lazar / Petar, on the live domain after the Y.07 deploy** |
