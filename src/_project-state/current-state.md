...
**Phase 2.25 — Impeccable pass — IN FLIGHT, NOT COMPLETE (branch `phase-2.25-impeccable-pass`, no PR,
not pushed, no preview).** Two of the four planned items are still owed on this same branch: **[P2]
`/impeccable polish`** and the **closing `/impeccable audit`**. Petar wants **one** PR and **one**
Vercel preview once all six land, so nothing here has reached production and none of it is verifiable
on the live site yet. **`CLAUDE.md`: do not cut a new branch while this one is unmerged.**
...
of their own and become client code when a client component imports them. Measured MK, both sides,
same dev server: **16,308 → 6,241 bytes** of serialized messages in the HTML of `/`, `/kontakt`,
`/uslovi` and `/katalog` alike — **−61.7%**; `Footer Faq About Terms Privacy ShippingReturns Catalog
...
**Petar's call with the cost in hand** (`D-2.25-10`): photos **138×173 → 288×360**, and the price moves
**y=567 → y=1107** at 320px — two screens of scroll to the buy path, past a hatched placeholder
(register **#2**); one `sm:` reverses it. `/styleguide`'s countdown demo **card now scrolls**
...
**Two build-pipeline traps found and documented in code:** a hand-written rule placed inside
`@layer base` **after `:focus-visible`**, and a rule added inside the existing
`@media (prefers-reduced-motion: reduce)` block **after the `*` selector**, are both **silently
dropped by the Tailwind v4 pipeline** — verified by serving the built CSS and grepping for the
selector while the class was present in the DOM and the rule present in source. Both are now authored
at the top level / in a second `@media` block (which the compiler merges back into one). Anyone
editing `globals.css` needs to know this.
...
diff. **Placeholder register unchanged.** New owed **#59–#63**. Decisions `D-2.25-5…14`.
