NEXT: **[P2] `/impeccable polish` + the closing `/impeccable audit` — on a NEW branch, whenever scheduled (`D-2.25-26`).** **Phase Y.06 — Composition & care wiring + removal of the 2-item order cap — CODE COMPLETE, pushed, PR [#41](https://github.com/petarjakimov11012011-cell/Trajanov-V2/pull/41) OPEN and NOT merged — awaiting the fresh-session review and the other operator's merge (`D-0-3`; `D-Y.06-1…9`).** **Two halves. (a) CARE WIRING — plumbing only, invisible on production:** new `src/lib/product-care.ts` (slug-keyed, mirroring `product-images.ts`) is read by the product page; all six `careMk`/`careEn` are still `null`, so the „Состав и нега" section renders **byte-identically to `main` in both locales — proven by diffing the rendered section HTML**, and placeholder register rows **#3/#9 stay OPEN**. `facts.md` §7 untouched; **no composition or care text of any kind is committed**, and `tests/config/product-care.test.ts` goes RED if any ever is (`D-Y.06-9`). **(b) THE 2-UNIT CAP IS GONE — Petar's call, 2026-08-27 (`D-Y.06-3`, supersedes `D-1.06-6`).** Removed from the cart module, `create_order()` step 3, the `order_items.quantity` CHECK, the standing line under the buy button, the cart summary banner, and four message keys × two catalogs; a **99-unit sanity ceiling** (`D-Y.06-4`) replaces it so `TR003` still refuses an absurd quantity cleanly. **A SEVENTH place the brief did not list was found by the closing grep and fixed — the Home FAQ answer `Faq.a3`, which also ships inside the FAQPage JSON-LD (`D-Y.06-8`).** **Named plainly: with no per-order cap, and cash on delivery meaning ordering costs nothing up front, one person can now take an entire drop having paid nothing** — real stock, the per-drop rate limit (`D-1.04-7`) and `TR005` are what remain. **Verified:** `npm run build && npm run lint && npx tsc --noEmit` all clean; **`npm test` 176/176 across 25 files** (166/24 before — +10, breakdown in the completion report); **10 simultaneous orders vs 3 units → exactly 3 succeed, stock 0**; **NEW: 5 simultaneous 3-unit orders vs one 3-unit variant → exactly 1 succeeds, 4 × TR004, stock 0, zero orphan rows**; a **real 3-unit order placed end to end through the MK checkout form on localhost (TRJ-0055, 3 × 1199 = 3597 MKD, one `order_items` row of quantity 3 — impossible before this phase)**; the migration applies on a **fresh chain**, on a **hand-restored pre-Y.06 database**, and **re-applied twice**, with the `create_order` signature and its EXECUTE grants (`postgres`, `service_role` only) intact each time. **Two new owed rows (#66, #67)** — native MK review of the four changed strings, and a real 3-unit order on `www.trajanovv.com` at the 2.06 rehearsal. Nothing on the critical path moved: **Y.01** (drop content load) is still the gate.
...
| 68 | **⚠ THE Y.06 MIGRATION MUST BE PUSHED TO HOSTED SUPABASE — this is an ACTION, not a check, and until it happens the deployed UI and the hosted database DISAGREE.** Merging Y.06 deploys a front end with **no cap message anywhere** — no standing line under the buy button, no cart banner, no Terms sentence, no FAQ answer — while hosted `create_order()` still asserts `1..2` and `order_items.quantity` still CHECKs `between 1 and 2`. **A customer would add 3 units, be told nothing, fill in the whole checkout form, and be refused at the last click** with „Провери ја количината во кошничката…" and no way to know what is wrong. `supabase db push` applies `20260827120000_remove_order_quantity_cap.sql`; it is the only unapplied migration. **Pass: on hosted, `\df+ create_order` shows the signature unchanged with EXECUTE for `service_role` only, and `order_items_quantity_check` reads `between 1 and 99`.** Note `D-1.07-15` — **never** `supabase db reset --linked` against a database with real orders. Owner: **Lazar**. | Y.06 | **immediately after merge, BEFORE any drop is opened** |
