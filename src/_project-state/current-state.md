NEXT: **[P2] `/impeccable polish` + the closing `/impeccable audit` — on a NEW branch (`D-2.25-26`).** **Phase Y.07 — Composition & care copy — COMPLETE, MERGED and LIVE (2026-08-27, PR [#42](https://github.com/petarjakimov11012011-cell/Trajanov-V2/pull/42), merge `36c08e5`, on Petar's explicit instruction — `D-Y.07-7`: operator-authorised per `D-0-3`, and GitHub records **no review at all** on the PR, so nobody should read "merged" as "reviewed"; branch deleted, refs pruned, one-branch rule re-armed; `D-Y.07-1…7`).** **Production VERIFIED on `https://www.trajanovv.com`:** all six product pages render the real care line and **no** placeholder element — MK „100% памук. Перење на 30 °C." ×3, EN "100% cotton. Wash at 30 °C." ×3, U+00A0 confirmed at codepoint level, one line at 390px in both locales, styled identically to the adjacent Shipping body; **zero `Product` JSON-LD nodes**; 19 routes swept 200. **⚠ ALSO DONE THIS SESSION: `supabase db push` was RUN** on Petar's instruction (`D-Y.07-6`) — the Y.06 migration is now on hosted and **`main` and the production database finally agree**. Verified by query: `schema_migrations` `20260716120000` → **`20260827120000`**, `create_order()` step 3 `> 2` → **`> 99`**, `order_items_quantity_check` `<= 2` → **`<= 99`**; applied with `orders` at **0 rows** and no drop open, so no customer was exposed. **Owed #68 CLOSED. Owed #69 CLOSED.** **Still open: #66** (native MK review of the four Y.06 strings), **#67** (a real 3-unit order end to end on production — needs a live drop), **#70** (native MK check of „100% памук. Перење на 30 °C." — check it together with #66), and **#65**. **Placeholder register is at 6 open rows** (#2, #4, #6, #7, #8, #10 — photos, names, courier + delivery cost, returns window) and **must reach zero before the first REAL drop**. **Never `supabase db reset --linked` (`D-1.07-15`).** Nothing on the critical path moved: **Y.01** (drop content load) is still the gate.
...
Last updated: **2026-08-27** · By: **Claude Code (Phase Y.07 — composition & care copy filled; `facts.md` §7 VERIFIED on the owner's statement; placeholder rows #3/#9 cleared; `D-Y.07-1…5`)**

---

## Status

**Phase Y.07 — Composition & care copy — COMPLETE, MERGED and LIVE (2026-08-27, PR
[#42](https://github.com/petarjakimov11012011-cell/Trajanov-V2/pull/42), merge `36c08e5`, on Petar's
explicit instruction — `D-Y.07-7`; branch deleted, refs pruned, one-branch rule re-armed; report:
`src/_project-state/completions/Part-Y-Phase-07-Completion.md`).** **GitHub records no review on the PR** —
`D-0-3` makes review an operator matter, and the merge was the operator's call, but the highest-risk thing
in this phase (a material claim about a physical garment, on a page selling cash-on-delivery) went to
production without a second human reading the diff. Code verified the strings byte-for-byte against
`facts.md` §7 and on all six live pages; Code checking its own work is not a second reader.

**`supabase db push` was RUN this session (`D-Y.07-6`), on Petar's instruction — the standing Y.06/hosted
mismatch is GONE.** `main` and the production database agree for the first time since 2026-08-27 morning.
Verified by reading the live database back, not by assuming the push worked. **Owed #68 and #69 are both
CLOSED**; #65, #66, #67 and the new #70 carry forward. The slot Y.06 shipped inert now carries a fact. Vladimir confirmed **100% cotton** and a
...
facts may render, and weight/GSM, drying, ironing, bleach, fibre origin, country of manufacture and every
adjective remain forbidden. The one statement was given as covering **all three colourways**, which
**amends `D-Y.02-1`** (`D-Y.07-2`, amended — not superseded). Three products × two locales are set
explicitly in `src/config/products.ts`; **no shared constant, no default, no fallback** (`D-Y.07-3`), so a
future fourth product cannot inherit a material claim nobody gave it. **Placeholder rows #3 and #9 are
CLEARED** — the first register rows to close since 2.05 — and **#2/#4/#6/#7/#8/#10 are untouched and still
OPEN**. **Nothing shipped to production by this phase:** care copy is config, not a DB column
(`D-Y.06-1`), so it reaches a customer only on the next deploy of `main` — owed **#69**.
...
### Composition & care copy (Y.07) — the slot is filled

- **The fact:** **100% cotton. Wash at 30 °C.** — and nothing else. Source: **Owner (Vladimir, via
  Petar), 2026-08-27**, recorded in `facts.md` §7 as VERIFIED with "**owner's statement, NOT a label
  read**" written into the row itself (`D-Y.07-1`). It covers **all three colourways** on the one
  statement, which **amends `D-Y.02-1`** (`D-Y.07-2`).
- **The strings** (`D-Y.07-3`, the only permitted care copy until `facts.md` says otherwise):
  MK `100% памук. Перење на 30\u00A0°C.` · EN `100% cotton. Wash at 30\u00A0°C.` — written with the
  `\u00A0` **escape** in the TS source so the non-breaking space survives copy-paste and shows in a diff.
  Set **per product** on all three; no shared constant, no default, no fallback.
- **`src/lib/product-care.ts` is byte-unchanged from `main`** — this phase supplied a value, it built
  nothing. `Placeholder.composition` stays in both catalogs and the null → placeholder branch stays in
  the product page: the next drop starts with null care fields and must still render an honest marker.
- **The Y.06 guard is replaced, not deleted** (`D-Y.07-4`, superseding `D-Y.06-9`(b);
  `D-Y.06-9`(a)'s blank-safe trim is untouched). `tests/config/product-care.test.ts` Group 2 is now an
  **exact-string allowlist**: every product's `careMk` is `null` or exactly the approved MK string,
  `careEn` is `null` or exactly the approved EN string, and the two are **both null or both non-null** —
  a composition in one language and a placeholder in the other is a defect. Group 1 (slug keying)
  unchanged.
- **Seen, not assumed.** All six pages (`/katalog/{test-mustard-ochre,test-off-white,test-baby-blue}` +
  `/en/catalog/…`) at **390px and 1180px**: placeholder gone from all six, correct locale on each,
  `careP.className === shipP.className` (`text-muted-foreground text-small`; measured 13px / 19.5px /
  `rgb(171,167,158)`, weight 400 — it reads as body copy, not a badge or a heading), and `30 °C` on **one
  line** (also one line at 320px). `productJsonLd` still emits **zero** `Product` nodes on all six.
- **Proven, not asserted:** 177/177 tests across 25 files (176/25 before); the new guard **watched
  failing** on a deliberately wrong string (`careMk: "100% cotton"` — wrong locale, EN words in the MK
  field) which tripped **both** new assertions, then reverted; `git diff` empty on
  `src/lib/product-care.ts`, `src/app/[locale]/catalog/[slug]/page.tsx`, `src/lib/seo/` and `supabase/`.
- **Not shipped yet.** Care copy is **config, not a DB column** (`D-Y.06-1`) — it does not exist for a
  customer until `main` is deployed, and `npm run sync:drop` will not carry it. Owed **#69**.
...
| ~~68~~ | ~~**THE Y.06 MIGRATION IS NOT ON HOSTED**~~ — **CLOSED 2026-08-27 (Y.07). `supabase db push` was RUN, on Petar's explicit instruction (`D-Y.07-6`), and the result read back from the live database.** Applied: exactly one migration, `20260827120000_remove_order_quantity_cap.sql` (dry-run first: no seeds, no roles; SQL read and confirmed non-destructive — `create or replace function` + one CHECK dropped by discovered name and re-added). **Verified after, by query, not assumption:** `supabase_migrations.schema_migrations` max **`20260716120000` → `20260827120000`**; `create_order()` step 3 **`v_total_qty > 2` → `> 99`**; `order_items_quantity_check` **`quantity <= 2` → `<= 99`**. Applied at the safest moment: `orders` **0 rows**, `test-drop` ended 2026-06-08, `open_now = false` — **no customer could be mid-checkout.** `main` and hosted now agree. **`supabase db reset --linked` was NOT used and never will be (`D-1.07-15`).** Row **#67** (a real 3-unit order end to end on a live drop) is untouched and still open. | Y.06 | **CLOSED — Code, verified against hosted 2026-08-27** |
| ~~69~~ | ~~**The two care strings on PRODUCTION, both locales, all three product pages**~~ — **CLOSED 2026-08-27 (Y.07), same day, after the merge deploy landed.** Verified on `https://www.trajanovv.com`, all **six** pages: the rendered „Состав и нега" / "Composition & care" section contains **only** the real string on every one — MK „100% памук. Перење на 30 °C." ×3, EN "100% cotton. Wash at 30 °C." ×3 — with **U+00A0 confirmed at codepoint level** (`0x33 0x30 0xa0 0xb0 0x43`) and **no `<Placeholder>` element rendered anywhere** (`/PLACEHOLDER/.test(section.innerText)` → `false`). **Note for the record:** the string `[PLACEHOLDER: состав и нега — од етикетата]` DOES still appear in the page source — in the serialized next-intl message catalog, i.e. the `Placeholder.composition` key we deliberately kept for the next drop. It is **not rendered**. A raw `grep` of the HTML will hit it; that is expected, not a regression. **Looked at on a real 390px viewport**, both locales (EN mustard/ochre, MK baby blue): one line, `careP.className === shipP.className` (`text-muted-foreground text-small`, computed 13px / `rgb(171,167,158)`), reads as body copy beside Shipping. **Zero `Product` JSON-LD nodes on all six.** 19 routes swept 200. | Y.07 | **CLOSED — Code, verified on the live domain 2026-08-27** |
| 70 | **Native MK check of „100% памук. Перење на 30 °C."** — a new customer-facing MK string that post-dates every native review (`mk-review-2.02/2.03/2.11`) and now renders on three live product pages. **Pass:** a native speaker reads it and confirms „Перење на 30 °C." is how the instruction is actually written on Macedonian care copy (not „Пери на 30 °C.", not „Мие се на 30 °C."), that lower-case „памук" is right, and that „100%" reads naturally tight against the word. **Row #66 (the four Y.06 strings) is the precedent and is still open — check them together.** | Y.07 | **Lazar** |
...
*Y.07 update (2026-08-27): **rows #3 and #9 CLEARED — the first two placeholder rows to close since 2.05.** Both are struck in place with a dated note, not deleted. What cleared them is Vladimir's statement of the composition and the wash temperature (**100% cotton, 30 °C**), relayed by Petar on 2026-08-27 and recorded in `facts.md` §7 as **VERIFIED — the owner's statement, explicitly NOT a label read** (`D-Y.07-1`); the one statement was given as covering **all three colourways**, which amends `D-Y.02-1` and is what closes #9 as well as #3 (`D-Y.07-2`). The two rendered strings are fixed by `D-Y.07-3` and enforced by an exact-string allowlist in `tests/config/product-care.test.ts` (`D-Y.07-4`, replacing `D-Y.06-9`'s "nothing committed" guard). **The `[PLACEHOLDER: состав и нега]` mechanism itself is NOT removed** — the key stays in both catalogs and the null → placeholder branch stays in the product page, because the next drop starts with null care fields and must still render an honest marker. (Unlike `Placeholder.email`, removed in 2.05: an email cannot become un-known, whereas a label can still contradict a recollection.) **Rows #2, #4, #6, #7, #8 and #10 are byte-unchanged and still OPEN** — photos, names, courier + delivery cost and the returns window remain Vladimir's, and the register must still reach zero before the first REAL drop.*
...
| ~~3~~ | ~~`[PLACEHOLDER: состав и нега — од етикетата]` (fabric/care)~~ | ~~Product~~ | **CLEARED 2026-08-27 (Y.07).** Vladimir confirmed the composition and the wash temperature — **100% cotton, 30 °C** — **via Petar, 2026-08-27**; `facts.md` §7 is now VERIFIED with that provenance written into the row, recorded explicitly as the **owner's statement, not a label read** (`D-Y.07-1`). `careMk`/`careEn` are filled on all three products, so the product page renders MK `100% памук. Перење на 30 °C.` / EN `100% cotton. Wash at 30 °C.` instead of this marker — verified on all six pages at 390px and 1180px. **The `Placeholder.composition` key and the null → placeholder branch both stay** (`D-Y.06-2`): the next drop starts null again and must still say so. **If a label is ever photographed and disagrees, this row reopens and `D-Y.07-1` is superseded.** | — |
...
| ~~9~~ | ~~`[PLACEHOLDER: состав и нега — од етикетата]` — **Product 03 (baby blue)** fabric/care~~ | ~~Product~~ | **CLEARED 2026-08-27 (Y.07).** Cleared by the **same owner statement** as #3 (Vladimir via Petar, 2026-08-27), which was given as covering **all three colourways** — so baby blue's fabric is no longer waiting on baby blue's own label. That **amends `D-Y.02-1`** (`D-Y.07-2`); it is the owner's word that retires the rule, **not** an inheritance from the other two shirts. `/katalog/test-baby-blue` and `/en/catalog/test-baby-blue` render the real line. **Baby blue's photo (#8) and real name (#10) are untouched and still OPEN.** | — |
