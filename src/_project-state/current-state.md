...
open to `main` — an operator merges (`D-0-3`); redeploy on merge.**
...
| Phase | **2.05 complete — Cutover** (`SITE_URL` → `https://www.trajanovv.com`; order email from `info@trajanovv.com`; email published on Contact both locales; delivery-time line + narrowed courier placeholder; Turnstile hostname confirmed non-asserting + site-key rotated; MK review stamped; `facts.md` §5/§7/§9; grep gate GREEN). Prior: **2.04b — SEO/GEO polish**; **2.04 — Perf, a11y, SEO**. Next: **2.06** (drop rehearsal + contingency) |
| Branch | `phase-2.05-cutover` → **PR open to `main`** (this session). Prior: `phase-2.04b-seo-geo-polish` → PR `#14`, merged `c562195` (2026-07-22); `phase-2.04-perf-a11y-seo` → PR `#13`, merged `6375a0d` (2026-07-20) |
| Open PR | **`phase-2.05-cutover` → `main` (2.05 cutover) — open this session; an operator merges (`D-0-3`).** Prior merged: 1.01–1.07 `#1`–`#7`; Z.01 `#8`; 1.08 `#9`; 2.01 `#10`; 2.02 `#11`; 2.03 `#12`; 2.04 `#13`; 2.04b `#14` |
| Deployed | **YES — served on `https://www.trajanovv.com`** (real custom domain, double-v; the apex `trajanovv.com` + `trajanov-v2.vercel.app` both 308-redirect to it). **2.05 (the `SITE_URL` flip → `www.trajanovv.com`, the `info@` from-address, the published contact email, the delivery-time line) is BUILT and PR-open, NOT yet deployed — it takes effect on the merge-triggered redeploy.** Until then the live build still emits `trajanov-v2.vercel.app` absolute URLs (2.04b). Prior smoke-verifications (2.04b/2.04/2.01) stand. `D-1.03-5`/`D-1.06-4` closed |
