...
| Branch | `phase-2.04b-seo-geo-polish` → PR **#14 MERGED to `main`** (merge `c562195`, 2026-07-22) on Petar's instruction; branch deleted. Prior: `phase-2.04-perf-a11y-seo` → PR `#13`, merged `6375a0d` (2026-07-20); `phase-2.03-legal-facts` → PR `#12`, merged `4fcc0bd` (2026-07-19) |
| Open PR | **None.** `#14` merged (2026-07-22). Prior merged: 1.01–1.07 `#1`–`#7`; Z.01 `#8`; 1.08 `#9`; 2.01 `#10`; 2.02 `#11`; 2.03 `#12`; 2.04 `#13`; 2.04b `#14` |
| Deployed | **YES — served on `https://www.trajanovv.com`** (a real custom domain, double-v; **`https://trajanov-v2.vercel.app` now 308-redirects to it** — attached outside the repo, Petar-confirmed his, `SITE_URL` left for 2.05). **2.04b is LIVE and smoke-verified by Code (2026-07-22):** `/llms.txt` (facts-clean, `noindex`), the Organization JSON-LD `logo` (`…/logo-512.png`, no address), and the `manifest`/`icon.svg`/`apple-icon` `<link>`s all serve; served absolute URLs still reference the vercel.app origin (SITE_URL-flip owed to 2.05). **2.04 is LIVE and smoke-verified by Code (2026-07-20):** `/sitemap.xml` (both locales + product entries), `/robots.txt`, the MK `/og` card (`image/png`), site Organization+WebSite JSON-LD, and an absolute `og:image` all serve; the product page ships **no** Product node (names still placeholders). **2.01 merged (`#10`) is LIVE and smoke-verified by Code (2026-07-19):** old `/catalog` **308→`/katalog`**, MK `/katalog` **200**, `/en/catalog` **200**, home `<html lang="mk">` + `canonical` + `hrefLang` mk/en/x-default all present. `D-1.03-5`/`D-1.06-4` closed |
| Domain | **⚠️ CHANGED — `trajanovv.com` (double-v) is LIVE and attached to Vercel** (`www.trajanovv.com`; `trajanov-v2.vercel.app` 308-redirects to it). Petar-confirmed his (2026-07-22). **`facts.md` §9 still says `trajanov.com` single-v "not purchased" — STALE, reconcile in 2.05.** `SITE_URL` NOT yet flipped (2.05) — see Known issues |
...
| 10 | **Live domain ≠ `SITE_URL` ≠ `facts.md`.** Production `trajanov-v2.vercel.app` **308-redirects to `https://www.trajanovv.com`** (double-v, Petar-confirmed his, discovered 2026-07-22 at the 2.04b merge — attached to Vercel outside the repo). But `SITE_URL` is still the vercel.app origin (Petar chose to defer the flip to 2.05), so **every absolute URL the site emits — canonical, hreflang, JSON-LD `logo`/`@id`, sitemap, OG images, llms.txt links — points at a host that redirects.** Search engines can treat a canonical that 308s as a soft signal; the OG/logo/IndexNow all assume the wrong host until fixed. And `facts.md` §9 still records `trajanov.com` **single-v** as "NOT YET PURCHASED". | `SITE_URL` (`src/lib/site.ts`) · `facts.md` §9 · line 1 | **Live — 2.05 must:** flip `SITE_URL` → `https://www.trajanovv.com`, reconcile `facts.md` §9 (spelling + purchased), redeploy, and register/ping IndexNow (owed #14). Not a build bug — a deliberate deferral, logged so 2.05 cannot miss it. |
