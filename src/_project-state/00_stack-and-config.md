...
| 2026-07-23 | 2.10 (Code) | **Product-card pointer spotlight — NO new dependency.** Four new design tokens added to `brand.md` (§3 `--color-glow`; §5 `--glow-size` `240px`, `--glow-opacity-surface` `0.05`, `--glow-opacity-edge` `0.22`) and mirrored into `src/app/globals.css` `:root` (same order, after `--color-mustard-tint-6`); only `--color-glow` is exposed as a utility via `@theme inline`, the other three are read directly through `var()`. A scoped `.spotlight-card` CSS block (gated in `@media (hover: hover) and (pointer: fine)`) lives in `globals.css`; the pointer tracking is a small client component (`src/components/product/SpotlightCard.tsx`). **No new npm dependency** (`package.json` + lockfile unchanged; `npm audit` advisories unchanged — pre-existing build-time postcss); no `config.toml`/Tailwind config/ESLint/tsconfig change (Tailwind v4 is CSS-first — there is no `tailwind.config`); `SITE_URL` untouched. The `brand.md` §5/§6 decoration+motion rules each gained a logged carve-out sentence (`D-2.10-1`). See `D-2.10-1…3`, `completions/Part-2-Phase-10-Completion.md`. | Claude Code |
| 2026-07-26 | Y.03 (Code) | **First images on the site — `next/image` now in use. NO new dependency.** `next/image` is built into `next 16.2.10`; nothing was installed (`package.json` + lockfile **unchanged**; `npm audit` advisories unchanged — the pre-existing build-time postcss pair). **Local files only:** two WebP under `public/images/lifestyle/` (`mustard-ochre-01.webp` 209 KB, `off-white-01.webp` 154 KB, both 1333×2000). **`next.config.ts` gets NO `images` block** — no remote pattern, no external host, no loader, no `unoptimized` flag; the default built-in optimiser serves them from `/_next/image`. Portability rule intact (nothing Vercel-native; the optimiser is Next's own and works on any Node host). Rendered through the existing `PhotoSlot` with `fill` + `object-cover` in the unchanged `aspect-[4/5]` box, `priority` **off**, `sizes="(min-width: 1024px) 280px, 50vw"`; verified in-browser that the served srcset makes the browser pick the **640px** candidate at 390px. New module `src/lib/product-images.ts` (slug→image map, `D-Y.03-1`). One new npm script: **none**. No `config.toml`/Tailwind/ESLint/tsconfig change; `SITE_URL` untouched; `create_order`/`expire_reservations`/`supabase/migrations/` untouched. **Standing note for the next phase:** if photography ever moves to a remote host, that needs an `images.remotePatterns` entry **and** a new decision — `D-Y.03-5` deliberately rejected a remote host. See `D-Y.03-1…11`, `completions/Part-2-Phase-Y03.md`. | Claude Code |
| 2026-07-27 | 2.21 (Code) | **Home showcase — NO dependency or config change.** `package.json` + lockfile **byte-unchanged** (diff-proven empty): the carousel is plain React state + CSS transitions (`D-2.21-1`); the control icons come from the already-installed `lucide-react 1.24.0` (`ArrowLeft`/`ArrowRight`/`Pause`/`Play` — the `HomeFaq` `Plus` precedent); the installed `motion 12.42.2` was deliberately NOT used (brief decision 10 — a cross-fade does not justify shipping it to the front door). No new npm script, env var, `config.toml`/Tailwind/ESLint/tsconfig change; `next.config.ts` untouched (**no remote image host** — the two slide photographs are the existing local Y.03 WebPs via the built-in optimiser); `SITE_URL` untouched. One CSS custom property (`--showcase-autoplay`) is set inline by the component from its own `AUTOPLAY_MS` constant — a component wiring value, **not** a `:root` token and **not** a `brand.md` row (`D-2.21-6`). `npm audit` advisories unchanged (the pre-existing build-time postcss pair via `next`). | Claude Code |
| 2026-08-27 | Y.06 (Code) | **Cap removal + care wiring — NO new dependency, no config change.** `package.json` + lockfile **unchanged**; no new npm script, no new env var, no `config.toml`/Tailwind/ESLint/tsconfig/`next.config.ts` change; `SITE_URL` untouched; `npm audit` advisories unchanged (the pre-existing build-time postcss pair via `next`). **One new migration — the first `supabase/migrations/` change since 2026-07-16:** `20260827120000_remove_order_quantity_cap.sql` re-creates `create_order()` with the **identical signature** (`text, text, text, text, text, text, jsonb, text, integer`) so its EXECUTE grants survive, changing exactly one executable line (step 3, `> 2` → `> 99`), and relaxes the `order_items.quantity` CHECK from `between 1 and 2` to `between 1 and 99` by dropping it **by discovered `pg_constraint` name** rather than an assumed one (`D-Y.06-5`). **Verified on a fresh chain, on a hand-restored pre-Y.06 database, and re-applied twice** — signature unchanged and grants (`postgres`, `service_role`; never `anon`/`authenticated`) intact each time. **This migration must be pushed to hosted Supabase before the next real drop** — until it is, hosted still refuses any order over 2 units with `TR003` while the deployed UI no longer says so. `src/lib/product-care.ts` makes `src/config/products.ts` **app-imported for the first time** (care copy only — drop state still comes from the DB, `D-1.04-9` intact), which means care copy changes need a **deploy**, not `npm run sync:drop`. See `D-Y.06-1…9`, `completions/Part-2-Phase-Y06-Completion.md`. | Claude Code |
