...
| **Email** | Resend | ***`Z.01`*** — **not built, no account, no key** (`D-1.07-8`). Supersedes the "*1.07*" pin, and the Cowork change-log row's "→ 1.08". Trigger: Vladimir's email exists (`facts.md` §5). | Order notification + customer confirmation. Free to 3,000/month — unreachable here. **A side channel, never the record.** |
...
| `RESEND_API_KEY` | server — **`Z.01`, not set** (`D-1.07-8`) | Vercel (when `Z.01` runs) |
| `ORDER_NOTIFICATION_EMAIL` | server — **`Z.01`, not set** (`D-1.07-8`); blocked on Vladimir's email (`facts.md` §5) | Vercel (when `Z.01` runs) |
